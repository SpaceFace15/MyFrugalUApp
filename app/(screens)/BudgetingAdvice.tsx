...
import { RootStackParamList, FinancialResource } from '../(tabs)/types'; 

type BudgetingAdviceNavigationProp = StackNavigationProp<RootStackParamList, 'BudgetingAdvice'>;

const BudgetingAdvice = () => {
  const navigation = useNavigation<BudgetingAdviceNavigationProp>();
      ...
      try {
        ...
        if (response.ok) {
          ...
          const filteredData = result.data.filter((resource: FinancialResource) => 
            resource.attributes.Category === 'Budgeting'
          );
  ...
  const baseUrl = 'http://192.168.254.113:1337'; //If debugging change IP to your network, do this by going in CMD and typing ipconfig
   ...
   const fullImageUrl = imageUrl ? `${baseUrl}${imageUrl}` : undefined;
     ...
     //Returning the URL on press the title and description
    return (
      <TouchableOpacity style={styles.card} onPress={() => openLink(item.attributes.Link)}>
        ...
        {imageUrl ? ( //Image style this will fetch the image and if somehow not available put a placeholder.
...
const styles = StyleSheet.create({
  ...
  card: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between', // Ensures items are spaced out to full width
    backgroundColor: '#fff',
    borderBottomWidth: 1,
...
export default BudgetingAdvice;
