...
const [housingExpenses, sethousingExpenses] = useState<any[]>([]);
...
const q = query(foodCol, where("expenseOwner", "==", auth.currentUser?.uid),
    where("exType", "==", "housingExpense")) // gets docs where it has current user id  and food in it


useLayoutEffect(() => {
    onSnapshot(q, (snapshot) => {
        let housing: { id: string; }[] = []

        snapshot.docs.forEach((doc) => {

            housing.push({ ...doc.data(), id: doc.id })
        })

        sethousingExpenses(housing);
    })
